...
pub async fn create_user_article(
    ...
    let id: i32 = path.into_inner();
    match sqlx::query_as::<_, Article>(
        "INSERT INTO ariticles (title, content, created_by) VALUES ($1, $2, $3) RETURNING id, title, content, created_by"
    )
    ...
    .await {
        ...
        Err(_) => HttpResponse::InternalServerError().json("Failed to creater user articles")
    }
}
