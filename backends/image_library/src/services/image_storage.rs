...
const BUCKET_NAME: &str = "hectorcrean";
...
fn get_aws_client(region: &str) -> Result<Client> {
    ...
    let key_id = env::var("AWS_ACCESS_KEY_ID").context("Missing S3_KEY_ID")?;
    let key_secret = env::var("AWS_SECRET_KEY").context("Missing S3_KEY_SECRET")?;
